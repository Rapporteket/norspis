...
dg <- readxl::read_xls("~/norspis/doc/NORSPIS - DG til sykehusveiviseren v2.xls", sheet = 1)

map_npr <- dg %>%
  dplyr::summarise(orgnr = paste(unique(orgnr), collapse = ", "),
            AvdNavn = paste(unique(AvdNavn), collapse = ", "),
            .by = ReshID) %>%
  rename(AvdRESH = ReshID) %>%
  filter(AvdRESH != "707383/4207041") %>%
  mutate(AvdRESH = as.numeric(AvdRESH)) %>%
  mutate(orgnr = as.numeric(orgnr)) %>%
  as_tibble()
map_npr <- bind_rows(map_npr,
                     map_avdresh_avdnavn[!(map_avdresh_avdnavn$AvdRESH %in% map_npr$AvdRESH), ])

#############

RegData$AvdNavn <- map_npr$AvdNavn[match(RegData$AvdRESH, map_npr$AvdRESH)]
...
data_to_resultatportalen_KI1_4_orgnr <-
  left_join(data_to_resultatportalen_KI1_4,
            dplyr::select(map_npr, -AvdRESH),
            by ="AvdNavn") %>%
  relocate(c("orgnr","var","denominator"), .before=AvdNavn ) %>%
  rename(year = "Year") %>%
  filter(orgnr %in% intersect(orgnr, dg$orgnr))

dgshusviser <- dg


# setdiff(map_npr$orgnr, data_to_resultatportalen_KI1_4_orgnr$orgnr)
# setdiff(data_to_resultatportalen_KI1_4_orgnr$orgnr, map_npr$orgnr)
# setdiff(data_to_resultatportalen_KI1_4_orgnr$orgnr, dg$orgnr)
# setdiff(dg$orgnr, data_to_resultatportalen_KI1_4_orgnr$orgnr)
