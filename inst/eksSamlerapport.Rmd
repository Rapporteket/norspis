...
# Datagrunnlag
Formålet med denne rapporten er å gi praktiske eksempler på hvordan slike rapporter kan etableres. Selv om datagrunnlaget godt kan være "ekte", så er nødvendigvis ikke resultatene som framkommer her anvendbare på annet vis en som metodisk illustrasjon.  
...
pasienter med > 1 forløp teller flere ganger.


# Totalt antall forløp fordelt på år
I datagrunnlaget ligger det registreringer fra og med `r format(min(df$HovedDato, na.rm = TRUE), "%d. %B, %Y")` til og med `r format(max(df$HovedDato, na.rm = TRUE), "%d. %B, %Y")` og består av i alt `r dim(df)[1]` forløp for i alt `r dplyr::distinct(df, PasientID) %>% dplyr::count()` pasienter.
...
```{r forlopPerAarFig, fig.cap = paste("Totalt antall forløp per år."), fig.pos = "H", fig.align = "center", out.width = "100%" }
ggplot2::ggplot(data = fYear, ggplot2::aes(x=Aar, y = Forlop)) +
  ggplot2::geom_bar(stat="identity")
```

# Typer av forløp type 1 og fordeling de siste 12 måneder
Fordeling av ulike typer forløp de siste 12 måneder er vist i Figur \@ref(fig:forlopPerMaanedFig).
